...
SidebarFollow.prototype = {
    ...
    _scrollScreen: function(args) {
        ...
        var bodyToTop = parseInt(jQuery('body')
            .css('top'), 10);
        if (!isNaN(bodyToTop)) {
            toTop += bodyToTop
        }
        var elementToTop = element.offset()
            .top - toTop;
        var referenceToTop = 0;
        if (prevElement && prevElement.length === 1) {
            referenceToTop = prevElement.offset()
                .top + prevElement.outerHeight()
        } else {
            referenceToTop = _self.cache.parentToTop - toTop
        }
        if (jQuery(document)
            .scrollTop() > elementToTop) {
            var elementHeight = element.outerHeight();
            _self.cache.placeholder.css('height', elementHeight)
